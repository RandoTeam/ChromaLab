...
| `VLM_MODEL_UNAVAILABLE` | Required chromatogram vision model is not installed, activated, or loadable before photo analysis. | Model readiness log, runtime evidence package, device/model metadata, validation run manifest. | Blocking | Yes after model install/activation | No until model is available | Yes |
