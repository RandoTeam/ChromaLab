# Skill: SKILL_03_IMAGE_VIEWER_ROI_EDITOR
...
Supports image viewer roi editor for ChromaLab orchestration. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.
...
An agent invokes `Image Viewer ROI Editor`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: SKILL_03_IMAGE_VIEWER_ROI_EDITOR
- Decision needed:
- Sources used:
