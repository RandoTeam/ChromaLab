# Skill: SKILL_01_KMP_COMPOSE_ARCHITECTURE

## Skill Name

KMP / Compose Architecture

## Skill Purpose

Supports kmp / compose architecture for ChromaLab orchestration. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- UI/shared architecture changes
...
- architecture plan + code changes
...
An agent invokes `KMP / Compose Architecture`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: SKILL_01_KMP_COMPOSE_ARCHITECTURE
- Decision needed:
- Sources used:
