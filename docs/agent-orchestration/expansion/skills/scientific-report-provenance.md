# Skill: scientific-report-provenance
...
Label report values by source: measured, calculated, inferred, OCR, VLM, user-confirmed, test-only. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Label report values by source: measured, calculated, inferred, OCR, VLM, user-confirmed, test-only.
...
An agent invokes `Scientific Report Provenance`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: scientific-report-provenance
- Decision needed:
- Sources used:
