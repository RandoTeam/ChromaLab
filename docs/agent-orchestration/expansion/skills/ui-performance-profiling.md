# Skill: ui-performance-profiling
...
Check recomposition, large bitmap handling, memory, frame pacing. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Check recomposition, large bitmap handling, memory, frame pacing.
...
An agent invokes `Ui Performance Profiling`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: ui-performance-profiling
- Decision needed:
- Sources used:
