...
Define user-facing uncertainty/status labels for scientific outputs. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Define user-facing uncertainty/status labels for scientific outputs.
...
An agent invokes `Uncertainty Labeling`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: uncertainty-labeling
- Decision needed:
- Sources used:
