...
Identify product risk, misleading output risk, and user harm/confusion risk. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Identify product risk, misleading output risk, and user harm/confusion risk.
...
An agent invokes `Product Risk Review`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: product-risk-review
- Decision needed:
- Sources used:
