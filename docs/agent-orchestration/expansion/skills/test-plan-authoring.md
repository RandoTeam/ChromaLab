...
Write phase test plans and regression checklists. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Write phase test plans and regression checklists.
...
An agent invokes `Test Plan Authoring`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: test-plan-authoring
- Decision needed:
- Sources used:
