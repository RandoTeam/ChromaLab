...
Design mobile task flows, friction points, recovery paths, and mode-specific workflows. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Design mobile task flows, friction points, recovery paths, and mode-specific workflows.
...
An agent invokes `Mobile Ux Flow Design`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: mobile-ux-flow-design
- Decision needed:
- Sources used:
