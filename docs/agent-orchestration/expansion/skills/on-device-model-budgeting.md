# Skill: on-device-model-budgeting
...
Define model size, timeout, caching, quantization/runtime constraints. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Define model size, timeout, caching, quantization/runtime constraints.
...
An agent invokes `On Device Model Budgeting`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: on-device-model-budgeting
- Decision needed:
- Sources used:
