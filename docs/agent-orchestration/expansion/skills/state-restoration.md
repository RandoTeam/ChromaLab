...
Persist and restore state across process death, rotation, and navigation. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Persist and restore state across process death, rotation, and navigation.
...
An agent invokes `State Restoration`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: state-restoration
- Decision needed:
- Sources used:
