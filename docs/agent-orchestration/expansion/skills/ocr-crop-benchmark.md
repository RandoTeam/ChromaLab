...
Benchmark OCR/VLM on tick labels, peak labels, title/ion text, low-resolution crops. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Benchmark OCR/VLM on tick labels, peak labels, title/ion text, low-resolution crops.
...
An agent invokes `Ocr Crop Benchmark`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: ocr-crop-benchmark
- Decision needed:
- Sources used:
