...
Review Android private/public storage, MediaStore, FileProvider, cleanup, and permissions. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Review Android private/public storage, MediaStore, FileProvider, cleanup, and permissions.
...
An agent invokes `Android Storage Privacy`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: android-storage-privacy
- Decision needed:
- Sources used:
