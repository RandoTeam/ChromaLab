# Skill: scientific-caveat-writing
...
Write precise caveats for review-grade, diagnostic, inferred, and user-confirmed values. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Write precise caveats for review-grade, diagnostic, inferred, and user-confirmed values.
...
An agent invokes `Scientific Caveat Writing`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: scientific-caveat-writing
- Decision needed:
- Sources used:
