# Skill: interaction-state-machine
...
Model user workflow states, transitions, blocked states, persistence, and rollback. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Model user workflow states, transitions, blocked states, persistence, and rollback.
...
An agent invokes `Interaction State Machine`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: interaction-state-machine
- Decision needed:
- Sources used:
