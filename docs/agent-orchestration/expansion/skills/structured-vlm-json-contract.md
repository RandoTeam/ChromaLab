# Skill: structured-vlm-json-contract
...
Create strict VLM prompt schemas and parse/validate outputs. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Create strict VLM prompt schemas and parse/validate outputs.
...
An agent invokes `Structured Vlm Json Contract`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: structured-vlm-json-contract
- Decision needed:
- Sources used:
