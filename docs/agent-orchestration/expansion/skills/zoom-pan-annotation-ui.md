...
Implement image annotation interactions: zoom, pan, draggable ROI boxes, handles, anchors. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Implement image annotation interactions: zoom, pan, draggable ROI boxes, handles, anchors.
...
An agent invokes `Zoom Pan Annotation Ui`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: zoom-pan-annotation-ui
- Decision needed:
- Sources used:
