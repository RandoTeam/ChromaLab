...
Define precise acceptance criteria per phase and feature. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Define precise acceptance criteria per phase and feature.
...
An agent invokes `Acceptance Criteria Design`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: acceptance-criteria-design
- Decision needed:
- Sources used:
