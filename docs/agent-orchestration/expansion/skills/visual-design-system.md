...
Define design tokens: colors, typography, spacing, shape, elevation, components. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Define design tokens: colors, typography, spacing, shape, elevation, components.
...
An agent invokes `Visual Design System`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: visual-design-system
- Decision needed:
- Sources used:
