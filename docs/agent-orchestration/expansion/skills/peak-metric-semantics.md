...
Validate meaning and reporting of RT, height, area, FWHM, S/N, asymmetry, resolution. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Validate meaning and reporting of RT, height, area, FWHM, S/N, asymmetry, resolution.
...
An agent invokes `Peak Metric Semantics`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: peak-metric-semantics
- Decision needed:
- Sources used:
