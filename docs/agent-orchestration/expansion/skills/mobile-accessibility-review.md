# Skill: mobile-accessibility-review
...
Check touch targets, scaling, focus order, screen reader labels, contrast. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Check touch targets, scaling, focus order, screen reader labels, contrast.
...
An agent invokes `Mobile Accessibility Review`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: mobile-accessibility-review
- Decision needed:
- Sources used:
