...
Rewrite technical errors into actionable user-facing language. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Rewrite technical errors into actionable user-facing language.
...
An agent invokes `Plain Language Errors`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: plain-language-errors
- Decision needed:
- Sources used:
