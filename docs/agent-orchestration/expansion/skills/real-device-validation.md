...
Run and document real Android device validation with artifact exports. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Run and document real Android device validation with artifact exports.
...
An agent invokes `Real Device Validation`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: real-device-validation
- Decision needed:
- Sources used:
