# Skill: evidence-gated-reporting
...
Design report gates so missing evidence cannot become release-quality output. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Design report gates so missing evidence cannot become release-quality output.
...
An agent invokes `Evidence Gated Reporting`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: evidence-gated-reporting
- Decision needed:
- Sources used:
