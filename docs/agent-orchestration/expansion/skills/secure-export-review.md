...
Review export/sharing behavior and safety. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Review export/sharing behavior and safety.
...
An agent invokes `Secure Export Review`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: secure-export-review
- Decision needed:
- Sources used:
