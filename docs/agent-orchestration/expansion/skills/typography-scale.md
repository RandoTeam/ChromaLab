...
Define type scale for mobile report, forms, overlays, and dense data tables. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Define type scale for mobile report, forms, overlays, and dense data tables.
...
An agent invokes `Typography Scale`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: typography-scale
- Decision needed:
- Sources used:
