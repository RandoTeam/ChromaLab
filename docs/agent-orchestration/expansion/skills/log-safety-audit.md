...
Ensure logs do not leak private data or huge artifacts. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Ensure logs do not leak private data or huge artifacts.
...
An agent invokes `Log Safety Audit`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: log-safety-audit
- Decision needed:
- Sources used:
