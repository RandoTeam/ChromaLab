# Skill: compose-kmp-implementation
...
Implement Compose/KMP UI components respecting state and design contracts. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Implement Compose/KMP UI components respecting state and design contracts.
...
An agent invokes `Compose Kmp Implementation`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: compose-kmp-implementation
- Decision needed:
- Sources used:
