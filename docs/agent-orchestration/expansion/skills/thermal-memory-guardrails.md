# Skill: thermal-memory-guardrails
...
Prevent memory spikes, model loops, thermal throttling, and UI hangs. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Prevent memory spikes, model loops, thermal throttling, and UI hangs.
...
An agent invokes `Thermal Memory Guardrails`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: thermal-memory-guardrails
- Decision needed:
- Sources used:
