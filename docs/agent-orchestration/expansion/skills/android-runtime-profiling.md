# Skill: android-runtime-profiling
...
Profile Android stages, memory, CPU, thermal behavior, and long-running jobs. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Profile Android stages, memory, CPU, thermal behavior, and long-running jobs.
...
An agent invokes `Android Runtime Profiling`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: android-runtime-profiling
- Decision needed:
- Sources used:
