...
Review Kovats/retention index requirements and valid/invalid computation conditions. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Review Kovats/retention index requirements and valid/invalid computation conditions.
...
An agent invokes `Kovats Ri Review`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: kovats-ri-review
- Decision needed:
- Sources used:
