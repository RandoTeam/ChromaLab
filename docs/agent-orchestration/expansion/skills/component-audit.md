...
Audit screens for default/generic styles, inconsistent spacing, poor hierarchy. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Audit screens for default/generic styles, inconsistent spacing, poor hierarchy.
...
An agent invokes `Component Audit`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
