...
Convert research into implementable project decisions with adoption/rejection rationale. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Convert research into implementable project decisions with adoption/rejection rationale.
...
An agent invokes `Research Synthesis`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: research-synthesis
- Decision needed:
- Sources used:
