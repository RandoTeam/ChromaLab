...
Compare output artifacts/overlays/JSON to expected golden files. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Compare output artifacts/overlays/JSON to expected golden files.
...
An agent invokes `Golden Artifact Testing`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: golden-artifact-testing
- Decision needed:
- Sources used:
