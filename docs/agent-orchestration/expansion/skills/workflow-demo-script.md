...
Define demo steps for validation by user/product owner. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Define demo steps for validation by user/product owner.
...
An agent invokes `Workflow Demo Script`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: workflow-demo-script
- Decision needed:
- Sources used:
