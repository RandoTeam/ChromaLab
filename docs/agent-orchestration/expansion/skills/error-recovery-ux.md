...
Design clear error states, retry/fallback paths, and diagnostic messaging. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Design clear error states, retry/fallback paths, and diagnostic messaging.
...
An agent invokes `Error Recovery Ux`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: error-recovery-ux
- Decision needed:
- Sources used:
