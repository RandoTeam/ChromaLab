...
Create scripts for manual validation on real devices with expected observations. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Create scripts for manual validation on real devices with expected observations.
...
An agent invokes `Usability Test Script`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: usability-test-script
- Decision needed:
- Sources used:
