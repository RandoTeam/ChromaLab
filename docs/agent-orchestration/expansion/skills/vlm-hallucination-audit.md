...
Identify where VLM invents values, geometry, labels, or confidence. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Identify where VLM invents values, geometry, labels, or confidence.
...
An agent invokes `Vlm Hallucination Audit`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: vlm-hallucination-audit
- Decision needed:
- Sources used:
