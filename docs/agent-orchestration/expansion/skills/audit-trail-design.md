...
Design durable audit record of all inputs, decisions, artifacts, and warnings. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Design durable audit record of all inputs, decisions, artifacts, and warnings.
...
An agent invokes `Audit Trail Design`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: audit-trail-design
- Decision needed:
- Sources used:
