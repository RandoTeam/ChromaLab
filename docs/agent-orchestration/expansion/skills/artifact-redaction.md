...
Remove or flag sensitive source filenames, paths, user data, logs, and images when needed. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Remove or flag sensitive source filenames, paths, user data, logs, and images when needed.
...
An agent invokes `Artifact Redaction`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: artifact-redaction
- Decision needed:
- Sources used:
