...
Create closure checklist for phases/features. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Create closure checklist for phases/features.
...
An agent invokes `Definition Of Done`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: definition-of-done
- Decision needed:
- Sources used:
