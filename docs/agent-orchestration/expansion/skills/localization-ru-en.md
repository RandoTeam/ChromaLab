...
Create Russian/English localization keys and verify terminology consistency. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Create Russian/English localization keys and verify terminology consistency.
...
An agent invokes `Localization Ru En`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: localization-ru-en
- Decision needed:
- Sources used:
