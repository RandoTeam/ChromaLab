# Skill: contrast-touch-target-audit
...
Audit visual contrast and tap target size. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Audit visual contrast and tap target size.
...
An agent invokes `Contrast Touch Target Audit`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: contrast-touch-target-audit
- Decision needed:
- Sources used:
