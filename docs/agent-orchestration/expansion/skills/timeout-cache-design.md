...
Add timeouts, caching, cancellation, fallback, and progress heartbeats. The skill converts a broad ChromaLab task into concrete decisions, artifacts, validation criteria, and safe handoff notes.

## When To Use

- Add timeouts, caching, cancellation, fallback, and progress heartbeats.
...
An agent invokes `Timeout Cache Design`, inspects active contracts, runs current research if the topic is unstable, compares options, writes a decision note, defines required artifacts, and hands off only after Orchestrator accepts the evidence plan.
...
```markdown
## Skill Handoff: timeout-cache-design
- Decision needed:
- Sources used:
