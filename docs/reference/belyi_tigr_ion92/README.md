...
Phase 1.12 makes the structured report the default calculation result surface. `AnalysisFlowScreen` now opens on the mobile `StructuredReportPreview` backed by the same `ChromatogramReport` contract used by export, while the older calculation summary remains available only as a diagnostics view. The main result path must keep following the reference-report section order instead of presenting the old summary as the final user report.
