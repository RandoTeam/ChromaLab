...
- [ ] Реализовать путь: ExtractedSignal → baseline → corrected signal → noise → peaks → boundaries → integration → metrics
- [ ] Считать RT, height, area, width, S/N
- [ ] Показывать результаты на интерактивном графике
- [ ] Дать пользователю вручную исправлять пики и границы
- [ ] Сохранять все параметры расчета
- [ ] Сохранять ручные корректировки
- [ ] Сохранять результаты в Room
- [ ] Обеспечить повторяемость: один и тот же сигнал + параметры + правки = тот же результат
- [ ] Не делать вывод "вещество найдено / не найдено" в этой фазе
- [ ] Все выводы по фото помечать как расчетные / ориентировочные

---

## 2.2 — Пользовательский сценарий Фазы 2

- [ ] Пользователь открывает оцифрованный сигнал из Фазы 1
- [ ] Видит график хроматограммы
- [ ] Видит переключатели слоев: raw / smoothed / baseline / corrected / peaks
- [ ] Нажимает "Найти пики"
- [ ] Приложение показывает найденные пики
- [ ] Пользователь нажимает на пик
- [ ] Открывается карточка пика с RT, height, area, width, S/N
- [ ] Пользователь может двигать левую и правую границу пика
- [ ] Пользователь может добавить пик вручную
- [ ] Пользователь может отклонить ошибочный пик
- [ ] Пользователь может выбрать noise region
- [ ] Пользователь может сменить baseline method
- [ ] После каждой правки метрики пересчитываются
- [ ] Пользователь сохраняет CalculationRun
- [ ] Пользователь экспортирует CSV/JSON результата
