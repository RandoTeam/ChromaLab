...
Status: `R15A_PLUS_TV2_LAYER_OWNER_BOARD_UPDATED`
...
| 8 Model / Knowledge assistance | `processing/inference`, `processing/model`, `feature/knowledge`, Android LiteRT/GGUF/model manager files, PC-only `tools/knowledge-retrieval` | model policy tests, Knowledge Pack tests, E2B acceptance matrix, TV-2 lexical vs TurboVec benchmark summary | `ACTIVE_IMPLEMENTATION`; E2B is advisory baseline. Knowledge retrieval has a facade plus active `LexicalKnowledgeRetrievalBackend`. TV-2 built PC-only TurboVec MiniLM/BGE indexes and found semantic improvements with 0 safety regressions, but dense profiles are not active runtime owners. | TV-3 must define the retrieval A/B or arbitration policy before TurboVec can replace lexical ranking. Android feasibility remains a later gate; model remains semantic only. |
...
| 5 | Stage 10 Knowledge retrieval / TurboVec | TV-2 proves local dense retrieval is promising for semantics/caveats, but it is not the direct blocker for current analyzer failures and needs TV-3 arbitration before promotion. |
