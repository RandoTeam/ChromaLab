...
Status: `R15A_PLUS_TV3_LAYER_OWNER_BOARD_UPDATED`
...
| 8 Model / Knowledge assistance | `processing/inference`, `processing/model`, `feature/knowledge`, Android LiteRT/GGUF/model manager files, PC-only `tools/knowledge-retrieval` | model policy tests, Knowledge Pack tests, E2B acceptance matrix, TV-2 lexical vs TurboVec benchmark summary, TV-3 arbitration summary | `ACTIVE_IMPLEMENTATION`; E2B is advisory baseline. Knowledge retrieval has a facade plus active `LexicalKnowledgeRetrievalBackend`. TV-3 selected `HYBRID_UNION_RRF` as the next benchmark target, but dense retrieval is not active runtime behavior yet. | TV-4 may implement a backend promotion candidate for the selected policy. Android feasibility remains a later gate; model remains semantic only. |
...
| 5 | Stage 10 Knowledge retrieval / TurboVec | TV-3 selected a safe hybrid benchmark target, but it is not the direct blocker for current analyzer failures and still needs TV-4 implementation proof before promotion. |
