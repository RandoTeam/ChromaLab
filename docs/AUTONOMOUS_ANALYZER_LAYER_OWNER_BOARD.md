...
| 5 | Stage 10 Knowledge retrieval / TurboVec | TV-6B proved on-device shell load/query behavior, but not app-private provider integration. Lexical remains active; TurboVec stays PC/dev-only or shell-probe-only until app-private storage, provider lifecycle, memory, latency, and citation safety are proven. |
