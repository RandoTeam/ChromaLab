...
object ReportHtmlRenderer {
    ...
    private fun StringBuilder.renderOverview(report: ChromatogramReport) {
        ...
        renderTable(
            ...
            rows = listOf(
                ...
                "Knowledge citations" to report.knowledgeCitations.size.toString(),
            ).map { listOf(it.first, it.second) },
        )
    }
    ...
    private fun StringBuilder.renderVisualEvidence(evidence: List<ReportVisualEvidenceContract>) {
        ...
        evidence.forEach { item ->
            val state = if (item.generatedStatus.isReadyVisualEvidence()) "ready" else "missing"
            appendLine("<span class=\"evidence $state\" data-evidence=\"${item.evidenceId.escapeHtml()}\">${item.label.escapeHtml()} (${item.generatedStatus.name.escapeHtml()})</span>")
        }
        appendLine("</div>")
    }
    ...
    private fun StringBuilder.renderTechnicalAppendix(
        ...
        appendLine("</section>")
        appendLine("<section data-section=\"knowledge_citations\">")
        appendLine("<h3>Knowledge Pack citations</h3>")
    ...
    private fun StringBuilder.renderKnowledgeCitations(report: ChromatogramReport) {
        if (report.knowledgeCitations.isEmpty()) {
            appendLine("<p class=\"muted\">No Knowledge Pack citations recorded.</p>")
            return
        }
        renderTable(
            headers = listOf(
            ...
            rows = report.knowledgeCitations.map { citation ->
                listOf(
                    citation.citationId,
        ...
        report.knowledgeCitations.forEach { citation ->
            appendLine("<h4>${citation.citationId.escapeHtml()}</h4>")
            renderTable(
                headers = listOf("Entry", "Type", "Claim scope", "Allowed use", "Forbidden use", "Trust tier", "Sources"),
                rows = citation.usedEntryRecords.map { record ->
                    listOf(
                        record.entryId,
                        ...
                        record.sourceRefs.joinToString("; ") { source ->
